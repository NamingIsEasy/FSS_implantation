...
class FCOSHead(nn.Module):
    ...
    def compute_loss(
        ...
        for targets_per_image, matched_idxs_per_image in zip(targets, matched_idxs):
            if len(targets_per_image["label"]) == 0:
                gt_classes_targets = targets_per_image["label"].new_zeros((len(matched_idxs_per_image),))
                gt_boxes_targets = targets_per_image["boxes"].new_zeros((len(matched_idxs_per_image), 4))
            else:
                gt_classes_targets = targets_per_image["label"][matched_idxs_per_image.clip(min=0)]
