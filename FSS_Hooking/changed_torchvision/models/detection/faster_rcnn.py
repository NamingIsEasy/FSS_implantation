...
class FasterRCNN(GeneralizedRCNN):
    ...
    containing:
        ...
        - label (Int64Tensor[N]): the class label for each ground-truth box
    ...
    follows:
        - boxes (``FloatTensor[N, 4]``): the predicted boxes in ``[x1, y1, x2, y2]`` format, with
          ``0 <= x1 < x2 <= W`` and ``0 <= y1 < y2 <= H``.
        - label (Int64Tensor[N]): the predicted label for each image
    ...
    def __init__(
        ...
        **kwargs,
        ...
        rpn = RegionProposalNetwork(
            ...
            residual_rpn_mode=kwargs['residual_rpn_mode']
        )

        if box_roi_pool is None:
            box_roi_pool = MultiScaleRoIAlign(featmap_names=["0", "1", "2", "3"], output_size=56, sampling_ratio=2)
        ...
        if box_predictor is None:
            representation_size = 1024
            box_predictor = FastRCNNPredictor(representation_size, num_classes, support_guide)
        ...
        roi_heads = RoIHeads(
            ...
            anchor_free_cluster_loss=kwargs['anchor_free_cluster_loss']
        ...
        super().__init__(backbone, rpn, roi_heads, transform, support_guide)
...
class FastRCNNPredictor(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, support_guide):
        super().__init__()
    ...
    def forward(self, x, box_ref_ele_features=None, box_ref_nee_features=None):
        if x.dim() == 4:
            torch._assert(
                list(x.shape[2:]) == [1, 1],
        ...
        if self.support_guide is True:
            ...
            ele_probability = torch.max(nn.CosineSimilarity(dim=-1, eps=1e-6)
                                        (torch.sum(box_ref_ele_features, dim=0).unsqueeze(0), x.unsqueeze(1)),
                                        dim=1)[0].unsqueeze(1)
            nee_probability = torch.max(nn.CosineSimilarity(dim=-1, eps=1e-6)
                                        (torch.sum(box_ref_nee_features, dim=0).unsqueeze(0), x.unsqueeze(1)),
                                        dim=1)[0].unsqueeze(1)
            bg_probability = torch.clamp(1-(ele_probability+nee_probability), min=0, max=1)
            scores = torch.cat([bg_probability, ele_probability, nee_probability], dim=1)
...
def fasterrcnn_resnet50_fpn(
    ...
    **kwargs: Any,
    ...
    follows, where ``N`` is the number of detections:
        ...
        - boxes (``FloatTensor[N, 4]``): the predicted boxes in ``[x1, y1, x2, y2]`` format, with
          ``0 <= x1 < x2 <= W`` and ``0 <= y1 < y2 <= H``.
        - label (``Int64Tensor[N]``): the predicted label for each detection
